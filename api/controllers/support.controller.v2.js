...
const normalizeAttachment = (fileOrMeta) => {
  if (!fileOrMeta) return null;
  const name = fileOrMeta.fileName || fileOrMeta.name || fileOrMeta.originalname || "file";
  const url = fileOrMeta.url || (fileOrMeta.filename ? `/uploads/${fileOrMeta.filename}` : "");
  const mimeType = fileOrMeta.mimeType || fileOrMeta.mimetype || "application/octet-stream";
  const size = Number(fileOrMeta.fileSize ?? fileOrMeta.size ?? 0);
  return { name, fileName: name, url, mimeType, size, fileSize: size, type: fileOrMeta.type || attachmentType(mimeType) };
};

const hydrateMessage = (m) => {
  const deleted = Boolean(m.deletedAt);
  return {
    id: Number(m.id), chatId: Number(m.chatId), conversationId: Number(m.conversationId || 0),
    senderId: Number(m.userId), userId: Number(m.userId),
    sender: { id: Number(m.userId), username: m.username, avatar: m.avatar, role: m.role },
    text: deleted ? "" : (m.text || ""), type: m.type || "TEXT",
    attachments: deleted ? [] : parseJson(m.attachments).map(normalizeAttachment).filter(Boolean),
    readReceipts: parseJson(m.readReceipts), isInternal: Boolean(m.isInternal),
    edited: Boolean(m.editedAt), deleted, deletedAt: m.deletedAt || null, createdAt: m.createdAt, updatedAt: m.updatedAt,
  };
};
...
const access = (req, alias = "sc") => {
  ...
  if (req.userRole === "STAFF") {
    if (!canManage(req)) return { sql: "1=0", params: [] };
    return { sql: `(${alias}.assignedToId = ? OR ${alias}.assignedToId IS NULL)`, params: [Number(req.userId)] };
  }
  if (req.userRole === "USER") return { sql: `${alias}.customerId = ?`, params: [Number(req.userId)] };
...
export const createCustomerConversation = async (req, res) => {
  try {
    ...
    try{
      ...
      const insertSql = `INSERT INTO support_conversations(chatId,customerId,propertyId,bookingId,type,status,subject,customerUnreadCount,staffUnreadCount,createdAt,updatedAt) VALUES(?,?,?,?,'CUSTOMER_SUPPORT','OPEN',?,0,?,NOW(3),NOW(3))`;
      await prisma.$executeRawUnsafe(insertSql, chat.id, customerId, validPropertyId, validBookingId, String(subject || "Support request").trim().slice(0, 255), clean ? 1 : 0);
      const inserted = await prisma.$queryRawUnsafe(`SELECT id FROM support_conversations WHERE chatId=? LIMIT 1`, chat.id);
      ...
      await notify(req, staff, "New support conversation", `A new customer support conversation #${id} is waiting.`, `/admin/support?conversation=${id}`);
...
export const createStaffConversation = async (req, res) => {
  if (!canManage(req)) return res.status(403).json({ message: "Support chat access required." });
  try {
    const customerId = Number(req.body?.customerId);
    if (!Number.isInteger(customerId) || customerId <= 0) return res.status(400).json({ message: "A customer is required." });
    const customer = await prisma.user.findFirst({ where: { id: customerId, isActive: true }, select: { id: true } });
    if (!customer) return res.status(404).json({ message: "Customer not found." });
    const text = typeof req.body?.initialMessage === "string" ? req.body.initialMessage.trim() : (typeof req.body?.text === "string" ? req.body.text.trim() : "");
    const subject = String(req.body?.subject || "Support request").trim().slice(0, 255);
    let validPropertyId = null;
    if (req.body?.propertyId != null) {
      const p = await prisma.property.findUnique({ where: { id: Number(req.body.propertyId) }, select: { id: true } });
    ...
    try {
      ...
      await prisma.$executeRawUnsafe(
        `INSERT INTO support_conversations(chatId,customerId,assignedToId,propertyId,type,status,subject,customerUnreadCount,staffUnreadCount,assignedAt,createdAt,updatedAt) VALUES(?,?,?,?, 'CUSTOMER_SUPPORT','OPEN',?,?,?,NOW(3),NOW(3),NOW(3))`,
        chat.id, customerId, Number(req.userId), validPropertyId, subject, text ? 1 : 0, 0,
      );
      const inserted = await prisma.$queryRawUnsafe(`SELECT id FROM support_conversations WHERE chatId=? LIMIT 1`, chat.id);
      const id = Number(inserted[0]?.id);
      if (!Number.isInteger(id) || id <= 0) throw new Error("support_conversations insert failed");
      if (text) {
        const m = await prisma.message.create({ data: { chatId: chat.id, userId: Number(req.userId), text } });
        await prisma.$executeRawUnsafe(`INSERT INTO support_message_meta(messageId,attachments,readReceipts,isInternal) VALUES(?,?,?,FALSE)`, m.id, JSON.stringify([]), JSON.stringify([{ userId: Number(req.userId), readAt: new Date().toISOString() }]));
        await prisma.chat.update({ where: { id: chat.id }, data: { lastMessage: text } });
        await prisma.$executeRawUnsafe(`UPDATE support_conversations SET customerUnreadCount=1, updatedAt=NOW(3) WHERE id=?`, id);
      }
      const conversation = await getConversation(req, id);
      await notify(req, [customerId], "Support conversation started", `A support specialist started conversation #${id}.`, `/support?conversation=${id}`);
      emitToUsers(req, [customerId, Number(req.userId)], "support:newConversation", { conversationId: id, conversation });
      return res.status(201).json(conversation);
    } catch (inner) {
      if (chat) await prisma.chat.delete({ where: { id: chat.id } }).catch(() => {});
      throw inner;
...
export const sendMessage = async (req,res)=>{
  try{const id=Number(req.params.id);const c=await getConversation(req,id);if(!c)return res.status(404).json({message:"Conversation not found."});const text=typeof req.body?.text==="string"?req.body.text.trim():"";const attachments=Array.isArray(req.body?.attachments)?req.body.attachments.map(normalizeAttachment).filter(Boolean):[];if(!text&&!attachments.length)return res.status(400).json({message:"Message text or attachment is required."});if(text.length>MAX_TEXT)return res.status(400).json({message:`Message must be ${MAX_TEXT} characters or fewer.`});if(c.status==="CLOSED"&&!canManage(req))return res.status(409).json({message:"This conversation is closed. Reopen it before replying."});
    ...
    const rows=await prisma.$queryRawUnsafe(`SELECT m.id,m.chatId,m.userId,m.text,m.createdAt,m.createdAt updatedAt,u.username,u.avatar,u.role,smm.attachments,smm.readReceipts,smm.isInternal,smm.deletedAt,smm.editedAt,? conversationId,CASE WHEN smm.attachments IS NOT NULL AND JSON_LENGTH(smm.attachments)>0 THEN 'ATTACHMENT' ELSE 'TEXT' END type FROM Message m JOIN User u ON u.id=m.userId LEFT JOIN support_message_meta smm ON smm.messageId=m.id WHERE m.id=?`,id,message.id);if(!rows[0])return res.status(500).json({message:"Failed to send message."});const saved=hydrateMessage(rows[0]);
    const staff=await activeSupportStaff();
    if(canManage(req)){await notify(req,[c.customerId],"Support replied",`You have a new reply in support conversation #${id}.`,`/support?conversation=${id}`);emitToUsers(req,[c.customerId,...staff,Number(req.userId)],"support:newMessage",{conversationId:id,message:saved,senderInfo:saved.sender});}
    else{const recipients=c.assignedToId?[c.assignedToId,...staff]:staff;if(wasUnread===0)await notify(req,recipients,"New customer message",`Customer ${c.customerName} sent a new support message.`,`/admin/support?conversation=${id}`);emitToUsers(req,[...recipients,c.assignedToId,Number(req.userId)],"support:newMessage",{conversationId:id,message:saved,senderInfo:saved.sender});}
...
export const editMessage=async(req,res)=>{
  try{
    ...
    const rows=await prisma.$queryRawUnsafe(`SELECT m.id,m.userId,smm.deletedAt FROM Message m JOIN support_conversations sc ON sc.chatId=m.chatId LEFT JOIN support_message_meta smm ON smm.messageId=m.id WHERE m.id=? AND sc.id=?`,messageId,id);
...
export const deleteMessage=async(req,res)=>{
  try{
    const id=Number(req.params.id),messageId=Number(req.params.messageId);
    const c=await getConversation(req,id);if(!c)return res.status(404).json({message:"Conversation not found."});
    const rows=await prisma.$queryRawUnsafe(`SELECT m.id,m.userId FROM Message m JOIN support_conversations sc ON sc.chatId=m.chatId WHERE m.id=? AND sc.id=?`,messageId,id);
    if(!rows.length)return res.status(404).json({message:"Message not found."});
...
export const addNote=async(req,res)=>{if(!canManage(req))return res.status(403).json({message:"Internal note access required."});try{const id=Number(req.params.id);if(!(await getConversation(req,id)))return res.status(404).json({message:"Conversation not found."});const body=String(req.body?.body||"").trim();if(!body)return res.status(400).json({message:"Note text is required."});await prisma.$executeRawUnsafe(`INSERT INTO support_notes(conversationId,authorId,body,pinned,createdAt,updatedAt) VALUES(?,?,?,?,NOW(3),NOW(3))`,id,Number(req.userId),body,req.body?.pinned?1:0);const inserted=await prisma.$queryRawUnsafe(`SELECT id FROM support_notes WHERE conversationId=? AND authorId=? ORDER BY id DESC LIMIT 1`,id,Number(req.userId));const noteId=Number(inserted[0]?.id);const note=(await getNotesForConversation(id)).find((n)=>n.id===noteId);emitToUsers(req,await activeSupportStaff(),"support:noteAdded",{conversationId:id,note});return res.status(201).json(note);}catch(e){console.error(e);return res.status(500).json({message:"Failed to add internal note."});}};
...
export const uploadAttachment=async(req,res)=>{
  try{
    ...
    return res.status(201).json(normalizeAttachment(req.file));
  }catch(e){console.error(e);return res.status(500).json({message:"Failed to upload attachment."});}
};
export const sendAttachment=async(req,res)=>{if(!req.file)return res.status(400).json({message:"File is required."});req.body={text:typeof req.body?.text==="string"?req.body.text:"",attachments:[normalizeAttachment(req.file)]};return sendMessage(req,res);};
