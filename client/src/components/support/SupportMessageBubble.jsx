 ...
 *
 * Features:
 *   - Renders text, images (inline), PDFs/files (download chip), audio (player)
 *   - Reply preview (quoted message)
 *   - Edited indicator
 *   - Deleted state (italic "message deleted")
 *   - Read receipts (single/double tick)
 *   - Hover actions: Reply, Forward, Edit (own), Delete, Copy, Download
 *   - Forwards indicator
 */
...
import {
  FiReply, FiForward, FiEdit2, FiTrash2, FiCopy, FiDownload,
  FiCheck, FiCheckCircle, FiMoreHorizontal, FiFile, FiPlay,
} from 'react-icons/fi';
      ...
      <div className="support-msg__col">
        ...
        {message.replyTo && (
          <div className="support-msg__reply-preview">
            ...
            <div>
              <small>Replying to message</small>
              <p>{message.replyTo.text || '[Attachment]'}</p>
            </div>
          </div>
        )}
        ...
        <div className="support-msg__bubble-wrap">
          ...
          {!isOwn && (
            <span className="support-msg__sender-name">{sender?.username}</span>
          )}
            ...
            {isDeleted ? (
              ...
              <>
                ...
                {message.attachments?.map((att, idx) => {
                  if (att.type === 'IMAGE') {
                    return (
                      <a
                        key={idx}
                        href={fullUrl(att.url)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="support-msg__image"
                      >
                        <img src={fullUrl(att.url)} alt={att.fileName} />
                      </a>
                    );
                  }
                  if (att.type === 'AUDIO') {
                    return (
                      <div key={idx} className="support-msg__audio">
                        <audio controls src={fullUrl(att.url)} style={{ width: '100%', maxWidth: 280 }} />
                        <small>{att.fileName} • {(att.fileSize / 1024).toFixed(0)} KB</small>
                      </div>
                    );
                  }
                  if (att.type === 'VIDEO') {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <small>{(att.fileSize / 1024).toFixed(0)} KB • {att.mimeType}</small>
                      </div>
                      <FiDownload size={14} />
                    </button>
                  );
                ...
                {message.forwardedFromId && (
                  <small className="support-msg__forwarded">↪ Forwarded</small>
                )}
              </>
            )}
          </div>
          ...
          {!isDeleted && (
            <div className="support-msg__actions">
              ...
              <button onClick={handleCopy} title="Copy">
                {copied ? <FiCheck size={12} color="#16a34a" /> : <FiCopy size={12} />}
              </button>
              {isOwn && (
                <button title="Edit" onClick={() => setMenuOpen(!menuOpen)}>
                  <FiEdit2 size={12} />
                </button>
              )}
              <div className="support-msg__more-wrapper" ref={menuRef}>
                ...
                {menuOpen && (
                  <div className="support-msg__menu">
                    <button onClick={() => { onReply?.(message); setMenuOpen(false); }}>
                      <FiReply size={12} /> Reply
                    </button>
                    <button onClick={() => { /* TODO: forward modal */ setMenuOpen(false); }}>
                      <FiForward size={12} /> Forward
                    </button>
                    {message.attachments?.map((att, i) => (
                      <button key={i} onClick={() => handleDownload(att)}>
                        <FiDownload size={12} /> Download {att.fileName.slice(0, 20)}
                      </button>
                    ))}
                    {isOwn && message.text && (
                      <button onClick={() => { /* TODO: edit */ setMenuOpen(false); }}>
                        <FiEdit2 size={12} /> Edit
                      </button>
                    )}
                    {(isOwn || true) && (
                      <button
                        onClick={() => { /* TODO: delete confirm */ setMenuOpen(false); }}
                        className="danger"
                      >
        ...
        <div className="support-msg__meta">
          ...
          {isOwn && !isDeleted && (
            <span className="support-msg__read-tick">
