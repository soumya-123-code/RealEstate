...
export default function HomePage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.allSettled([apiRequest.get('/cms/banners'), apiRequest.get('/properties?isFeatured=true&limit=6')])
      .then(([bannerResult, propertyResult]) => {
        if (!active) return;
  ...
  return (
    <main className="homePage">
      <section className="hero-section" aria-label="Find your property">
        ...
        <div className="hero-content">
          ...
          <div className="hero-buttons">
            <Link to={heroTarget} className="hero-btn primary">Explore Properties <FiArrowRight /></Link>
            <Link to="/chat" className="hero-btn secondary"><FiMessageCircle /> Contact an Agent</Link>
          </div>
          <form className="hero-search" onSubmit={(event) => {
            ...
            navigate(params.toString() ? `/list?${params}` : '/list');
          }}>
      ...
      <section className="trust-section"><div className="container trust-grid"><div><FiShield /><strong>Trusted listings</strong><span>Property information comes from Suretreaven's managed inventory.</span></div><div><FiMapPin /><strong>Clear property details</strong><span>Search location, type, price and key property information.</span></div><div><FiMessageCircle /><strong>Talk to an agent</strong><span>Ask questions before you make a booking.</span></div></div></section>
