...
const PROPERTY_STATUSES = new Set(["AVAILABLE", "TOKEN_BOOKED", "SOLD", "RENTED", "LEASED", "UNAVAILABLE", "UNDER_CONSTRUCTION"]);
...
export const updateAgentProperty = async (req, res) => {
  try {
    ...
    if (!Number.isInteger(id)) return res.status(400).json({ message: "Invalid property." });
    const property = await prisma.property.findFirst({ where: { id, agentId: agent.id }, select: { id: true } });
    if (!property) return res.status(404).json({ message: "Property not found." });
    const data = {};
    if (req.body.status !== undefined) {
      if (!PROPERTY_STATUSES.has(req.body.status)) return res.status(400).json({ message: "Invalid property status." });
      data.status = req.body.status;
    }
    if (req.body.price !== undefined) {
      const price = Number(req.body.price);
      if (!Number.isFinite(price) || price < 0) return res.status(400).json({ message: "Invalid property price." });
      data.price = price;
    }
