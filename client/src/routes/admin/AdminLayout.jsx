...
const PAGE_TITLES = {
  '/admin': 'Dashboard', '/admin/properties': 'Properties', '/admin/bookings': 'Bookings',
  '/admin/users': 'Users', '/admin/agents': 'Agents', '/admin/chat': 'Chat', '/admin/settings': 'Settings',
};
const NAV = [
...
function AdminLayout() {
  ...
  useEffect(() => {
    if (currentUser && canAccessAdminPanel()) apiRequest.get('/company/settings').then((res) => setCompanySettings(res.data)).catch(() => {});
  ...
  const handleLogout = async () => { try { await apiRequest.post('/auth/logout'); } catch {} logout(); navigate('/admin/login'); };
    ...
    <div className="admin-layout">
      ...
      <aside className={`admin-sidebar ${mobileSidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          {NAV.map(({ to, end, icon: Icon, label }) => <NavLink key={to} to={to} end={end} className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`} onClick={() => setMobileSidebarOpen(false)}><span className="nav-icon"><Icon size={16} /></span><span>{label}</span></NavLink>)}
          <div className="nav-group nav-group--bottom"><a href="/" className="nav-link"><span className="nav-icon"><FiEye size={16} /></span><span>View Site</span></a><button onClick={handleLogout} className="nav-link nav-link--logout"><span className="nav-icon"><FiLogOut size={16} /></span><span>Logout</span></button></div>
        </nav>
      </aside>

      <div className="admin-main">
        <header className="admin-topbar">
          <button className="mobile-toggle" onClick={() => setMobileSidebarOpen((v) => !v)} aria-label="Toggle sidebar">{mobileSidebarOpen ? <FiX size={20} /> : <FiMenu size={20} />}</button>
          <div className="topbar-breadcrumb"><span className="breadcrumb-home">Admin</span><FiChevronRight size={13} /><span className="breadcrumb-current">{title}</span></div>
          <div className="topbar-right">
            <div className="admin-notif"><button className="topbar-icon-btn" onClick={() => setNotifOpen((v) => !v)} aria-label="Open notifications"><FiBell size={17} />{totalUnread > 0 && <span className="notif-badge">{totalUnread > 99 ? '99+' : totalUnread}</span>}</button>{notifOpen && <div className="notif-dropdown"><div className="notif-header"><strong>Notifications</strong></div><div className="notif-list">{chatNotifications?.length ? chatNotifications.slice(0, 5).map((n) => <button key={n.chatId} className="notif-item" onClick={() => { setNotifOpen(false); navigate('/admin/chat'); }}><span>{n.senderName || 'New message'}</span><small>{n.lastMsg || 'You have a new message'}</small></button>) : <div className="notif-empty"><p>No new notifications</p></div>}</div></div>}</div>
            <div className="topbar-user"><div className="topbar-avatar">{currentUser.avatar ? <img src={currentUser.avatar} alt={currentUser.username} /> : <span>{initials}</span>}</div><div className="topbar-user-text"><span className="topbar-user-name">{currentUser.username}</span><span className="topbar-user-role">{isAdmin() ? 'Administrator' : 'Staff'}</span></div></div>
            <button className="topbar-logout-btn" onClick={handleLogout}><FiLogOut size={16} /><span>Logout</span></button>
          </div>
        </header>
