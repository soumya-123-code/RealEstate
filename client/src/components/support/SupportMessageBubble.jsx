...
import {
  FiCornerUpLeft, FiShare, FiEdit2, FiTrash2, FiCopy, FiDownload,
  FiCheck, FiCheckCircle, FiMoreHorizontal, FiFile,
} from 'react-icons/fi';
import './SupportMessageBubble.scss';

function SupportMessageBubble({ message, isOwn, conversationId, currentUserId, onReply, onMarkRead }) {
  ...
  const handleCopy = () => { navigator.clipboard.writeText(message.text || ''); setCopied(true); setTimeout(() => setCopied(false), 1500); setMenuOpen(false); };
  const handleDownload = (att) => { const a = document.createElement('a'); a.href = fullUrl(att.url); a.download = att.fileName; a.target = '_blank'; a.click(); };
  const sender = message.sender;
  const avatarUrl = sender?.avatar ? (sender.avatar.startsWith('http') ? sender.avatar : `${apiBase}${sender.avatar}`) : null;

  return (
    <div className={`support-msg ${isOwn ? 'own' : 'other'} ${isDeleted ? 'deleted' : ''}`}>
      {!isOwn && <div className="support-msg__avatar">{avatarUrl ? <img src={avatarUrl} alt={sender?.username} /> : <span>{sender?.username?.[0]?.toUpperCase() || '?'}</span>}</div>}
      <div className="support-msg__col">
        {message.replyTo && <div className="support-msg__reply-preview"><div className="support-msg__reply-bar" /><div><small>Replying to message</small><p>{message.replyTo.text || '[Attachment]'}</p></div></div>}
        <div className="support-msg__bubble-wrap">
          ...
          <div className={`support-msg__bubble ${isOwn ? 'own' : 'other'}`}>
            {isDeleted ? <em className="support-msg__deleted-text">🗑️ This message was deleted</em> : <>
          ...
          {!isDeleted && <div className="support-msg__actions">
            ...
            <div className="support-msg__more-wrapper" ref={menuRef}>
              ...
              {menuOpen && <div className="support-msg__menu">
                ...
                <button onClick={() => setMenuOpen(false)}><FiShare size={12} /> Forward</button>
        ...
        <div className="support-msg__meta"><small>{format(message.createdAt)}</small>{isEdited && <small className="support-msg__edited">edited</small>}{isOwn && !isDeleted && <span className="support-msg__read-tick">{isRead ? <FiCheckCircle size={12} /> : <FiCheck size={12} />}</span>}</div>
      </div>
    </div>
  );
}
